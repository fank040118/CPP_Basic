...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (cin.fail())
        {
            ...
            // code goes error when user input is not a int
            // infinite loop because the code did not clear the fail flag, while loop is keeping repeating by cin.fail()
            {
                cout << "Expecting a number as the option!" << endl;
                validInput = false;
                cin.clear(); // solution is add cin.clear() to clear the flag when input is not a int and != q || Q
            }
        }
